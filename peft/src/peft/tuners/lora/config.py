...
class MonteCLoRAConfig:
    def __init__(self, 
                 ...
                 kl_loss_weight=1e-5, 
                 mc_training=True):
        self.use_monteclora = use_monteclora
        self.monteclora_at = monteclora_at
        self.monteclora_targets = (
            set(monteclora_targets) if isinstance(monteclora_targets, list) else monteclora_targets 
        )
    ...
    def __repr__(self):
        return (f"MonteCLoRAConfig(use_monteclora={self.use_monteclora}, monteclora_at='{self.monteclora_at}', "
                f"monteclora_targets='{self.monteclora_targets}', monteclora_n={self.monteclora_n}, monteclora_m={self.monteclora_m} "
                f"sample_scaler={self.sample_scaler}, use_entropy={self.use_entropy}, "
                f"kl_loss_weight={self.kl_loss_weight}, "
                f"mc_training={self.mc_training})")
...
class LoraConfig(PeftConfig):
    ...
    use_monteclora: bool = field(default=False, metadata={"help": "use monteclora or not"})
    monteclora_at: str = field(default="lora_A", metadata={"help": "LoRA modules to apply monteclora"})
    monteclora_targets: str = field(default = 'query',metadata={"help": "Transformer layers to apply monteclora"})
    monteclora_n: int = field(default=4, metadata={"help": "Number of Samples from each gaussian"})
    monteclora_m: int = field(default=4, metadata={"help": "Number of Gaussians"})
    sample_scaler: float = field(default= 5e-3 , metadata={"help": "To scale down the magnitude of vvariational loss"})
    use_entropy: bool = field(default=False, metadata={"help": "True if entropy loss is needed to enforce cooperation"})
    dirichlet_prior: int = field(default = 1, metadata={"help":"Take samples at every nth (sample period) step"})
    kl_loss_weight: float = field(default=1e-5, metadata={"help": "KL loss weight"})
    mc_training: bool = field(default=True, metadata={"help": "Whether to train the monteclora module"})
