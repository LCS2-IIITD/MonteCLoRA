...
def main(
    ...
    for instruction in [
        ...
        print()
    """
    save_file = f'experiment/{args.model}-{args.adapter}-{args.dataset}-{args.tag}.json'
...
def parse_args():
    ...
    parser.add_argument('--dataset', choices=['AddSub', 'MultiArith', 'SingleEq', 'gsm8k', 'AQuA', 'SVAMP'],
                        required=True)
    parser.add_argument('--model', choices=['LLaMA-7B', 'LLaMA-1B', 'BLOOM-7B', 'GPT-j-6B'], required=True)
