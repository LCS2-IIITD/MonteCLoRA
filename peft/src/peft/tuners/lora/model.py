    ...
    **Attributes**:
    ...
    def _create_and_replace(
        ...
        kwargs = {
            ...
            "current_key": current_key, # required for MonteCLoRA as we treat it as a hitch-hiker on LoRA modules
        }
        ...
        if isinstance(target, LoraLayer) and not isinstance(target, AdaLoraLayer):
            target.update_layer( # these come from def above, regex matched
                adapter_name,
                r,
                lora_alpha=alpha,
                lora_dropout=lora_config.lora_dropout,
    ...
    def _create_new_module(lora_config, adapter_name, target, **kwargs):
        ...
        if current_key.split('.')[-1] in monteclora_targets:
            if lora_config.monteclora_config.use_monteclora:
                kwargs.update(
                    {
                        "use_monteclora": True,
