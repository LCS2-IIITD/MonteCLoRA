...
class AsyncMonteCLoRASampler(threading.Thread):
    def __init__(self, model, buffer_size=1, device='cpu'):
        ...
        self.running = True
        self.mvn_sampler = MultivariateNormal(loc=torch.zeros(self.model.out_features).to(self.device), \
                                     covariance_matrix=torch.eye(self.model.out_features).to(self.device))
        self.dir_sampler = MultivariateNormal(loc=torch.zeros(self.model.monteclora_m).to(self.device), \
                                     covariance_matrix=torch.eye(self.model.monteclora_m).to(self.device))
        self.wishart_sampler = Wishart(
    ...
    def run(self):
        while self.running:
            if self.queue.qsize() < self.buffer_size:
                ...
                try:
                    with torch.no_grad():
                        z_mvn = self.mvn_sampler.sample((self.model.monteclora_n * self.model.monteclora_m, self.model.in_features)).to(self.device)
                        z_mvn = z_mvn.view(self.model.monteclora_m,self.model.monteclora_n, self.model.in_features, self.model.out_features)
                        z_wishart = self.wishart_sampler._bartlett_sampling(torch.Size([self.model.monteclora_m])).to(self.device)
                        z_dirichlet = self.dir_sampler.sample().to(self.device)
...
class MonteCLoRASampler(nn.Module):
    def __init__(
        ...
        self.monteclora_m = monteclora_m
        ...
        self.std_prior = nn.Parameter(torch.rand(monteclora_m, out_features))
        self.expert_weights_prior = nn.Parameter(torch.rand(monteclora_m))
        self.gaussian_var_prior = torch.eye(out_features).to(device)
        self.expert_weights = torch.ones(monteclora_m, device=device) / monteclora_m
        self.fwd_counter = 0
        self.start_update_at = start_update_at
        self.sampler = None
        if self.mc_training:
    ...
    def wishart_reparameterization(self, std, z_wishart):
        updated_var = torch.clip(torch.bmm(torch.bmm(std, z_wishart), std.transpose(1, 2)), min = eps)
        # updated_var = torch.diag_embed(torch.clip(updated_var.diag(), min=eps).view(self.m, self.out_features))
        # print(updated_var.shape)
        return torch.diag_embed(torch.diagonal(updated_var, dim1=1, dim2=2))

    def multivariate_reparameterization(self, z_mvn, cov_matrix):
        """Sample a random variable with covariance ``cov_matrix``.
        On CUDA, ``torch.linalg.cholesky`` has no FP16 kernel, so we cast to
        FP32 just for the factorisation and immediately cast back. """

        with torch.amp.autocast('cuda', enabled=False):
            L = torch.linalg.cholesky(cov_matrix.float())
        L = L.to(cov_matrix.dtype)

        varsum = torch.mean(torch.einsum('mnir,mrk->mnik', z_mvn, L), dim=1)
    ...
    def multivariate_kl(self, var):
        ...
        # Convert to float32 for operations that don't support float16
        var_float = var.float()
        
        var_trace = torch.diagonal(var_float, dim1=1, dim2=2).sum(dim=1)
        log_var_trace = torch.logdet(var_float)
        kl_div = self.monteclora_n * 0.5 * (var_trace - log_var_trace - self.out_features)  # Shape: (m)
        
        return kl_div.mean()

    def dirichlet_reparameterization(self, alpha, z_dirichlet):
        mu = torch.log(alpha) - torch.log(alpha).mean()
        sigma = torch.diag(1/alpha * (1 - 2/self.monteclora_m) + 1/(self.monteclora_m**2) * (1/alpha).sum())
    ...
    def dirichlet_kl(self, alpha2):
        alpha1 = torch.tensor([self.dirichlet_prior]*self.monteclora_m, device=self.device)
    ...
    # std has shape (m, r, r)
    # We need to handle batch of matrices
    
    # Calculate var for each matrix in the batch
        var = torch.bmm(std, std.transpose(1, 2))  # Shape: (m, r, r)
        
        # Extract diagonals and create diagonal matrices
        diag_values = torch.diagonal(var, dim1=1, dim2=2)  # Shape: (m, r)
        var = torch.diag_embed(diag_values)  # Shape: (m, r, r)
        
        # Calculate trace of log(var) for each matrix
        log_var = torch.log(diag_values)  # Shape: (m, r)
        log_var_trace = torch.sum(log_var, dim=1)  # Shape: (m)
        
        # Calculate trace of var for each matrix
        var_trace = torch.sum(diag_values, dim=1)  # Shape: (m)
        
        # Calculate KL divergence for each matrix
        kl_div = 0.5 * (-log_var_trace * self.out_features + 
                        var_trace * self.out_features - 
                        self.out_features**2)  # Shape: (m)
    
        return kl_div.mean()  # Returns an array of KL divergences, one per matrix

    # def multivariate_kl(self, var):
    # # var has shape (m, r, r)
    #     var = torch.clamp(var, min=1e-6)
    #     var_trace = torch.diagonal(var, dim1=1, dim2=2).sum(dim=1)
    #     log_var_trace = torch.logdet(var)
    #     kl_div = self.monteclora_n * 0.5 * (var_trace - log_var_trace - self.out_features)  # Shape: (m)
        
    #     return kl_div.mean()

    def get_variational_loss(self):
        if self.mc_training and self.training and self.fwd_counter > self.start_update_at:
            kl1 = self.dirichlet_kl(torch.exp(self.expert_weights_prior))
            kl2 = self.wishart_kl(torch.diag_embed(torch.exp(self.std_prior)))
            ...
            return self.kl_loss_weight * (kl1 + kl2 + kl3) / (self.monteclora_m * self.monteclora_n), entropy
        return 0, 0

    def forward(self):
        self.fwd_counter += 1
        if self.training and self.mc_training and self.fwd_counter > self.start_update_at:
            ...
            if sample is not None:
                ...
                z_mvn = MultivariateNormal(loc=torch.zeros(self.out_features).to(self.device), \
                                     covariance_matrix=torch.eye(self.out_features).to(self.device)).sample((self.monteclora_n * \
                                                                            self.monteclora_m, self.in_features)).to(self.device)
                z_mvn = z_mvn.view(self.monteclora_m,self.monteclora_n, self.in_features, self.out_features)
                wishart_sampler = Wishart(
                    df=self.out_features,
                    scale_tril=torch.eye(self.out_features, device=self.device)
                )
                z_wishart = wishart_sampler._bartlett_sampling(torch.Size([self.monteclora_m]))
                z_dirichlet = MultivariateNormal(loc=torch.zeros(self.monteclora_m).to(self.device), \
                                     covariance_matrix=torch.eye(self.monteclora_m).to(self.device)).sample().to(self.device)

            std = torch.diag_embed(torch.exp(self.std_prior))
...
#         self.expert_weights = (torch.ones(monteclora_n)/ monteclora_n) 
...
#         all_vars = sampler.sample((self.monteclora_n, self.in_features)).to(self.device)
...
#         sampler = MultivariateNormal(loc=torch.zeros(self.monteclora_n).to(self.device), \
#                                      covariance_matrix=torch.eye(self.monteclora_n).to(self.device))
#         sample = sampler.sample().to(self.device)
#         mu = torch.log(alpha2) - 1/self.monteclora_n * torch.log(alpha2).sum()
#         sigma = torch.diag(1/alpha2 * (1 - 2/self.monteclora_n) + 1/(self.monteclora_n ** 2) * (1/alpha2).sum())
#         return torch.linalg.cholesky(sigma) @ sample + mu
        
#     def dirichlet_kl(self, alpha2):
#         # https://statproofbook.github.io/P/dir-kl.html
#         alpha1 = torch.tensor([self.dirichlet_prior]*self.monteclora_n).to(self.device)
