...
def main(
    ...
    assert os.path.exists(args.lora_weights) == True
    save_files2 = os.path.join(args.lora_weights,'results_' + args.dataset + '.txt' )
...
def parse_args():
    ...
    parser.add_argument('--adapter', choices=['LoRA', 'AdapterP', 'AdapterH', 'Parallel', "MonteCLoRA"],
                        required=True)
...
def load_model(args) -> tuple:
    ...
    for module in list(dict(model.named_modules()).values()):
        if type(module).__name__ == 'CooperativeLinear' or type(module).__name__ == 'CooperativeConv1D'\
            or type(module).__name__ == 'MonteCLoRASampler':
            module.eval()
...
def extract_answer(args, sentence: str) -> float:
    ...
    if dataset == "boolq":
        ...
        # Look for yes/no/true/false, case-insensitive, matching whole words only
        matches = re.findall(r"\b(true|false|yes|no)\b", sentence_, flags=re.IGNORECASE)

        if not matches:
            return ""

        answer = matches[0].lower()     # normalise case first

        # Map yes→true and no→false, leave true/false unchanged
        return "true"  if answer in {"yes", "true"} else "false"
...
# --lora_weights '/home/vaibhav/MonteCLoRA/expt-runs/lora/llama-3.2-3B-lora-test-cs15k/checkpoint-5625' \
# --batch_size 8