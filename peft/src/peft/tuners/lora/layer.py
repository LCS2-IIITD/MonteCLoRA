...
from peft.tuners.tuners_utils import BaseTunerLayer, check_adapters_to_merge, MonteCLoRASampler, MonteCLoRALinear
...
class LoraLayer(BaseTunerLayer):
    ...
    def update_layer(
        ...
        if "monteclora_config" in self.kwargs:
            monteclora_config = self.kwargs["monteclora_config"]
            self.use_monteclora = True if monteclora_config.use_monteclora else False
            if "lora_A" in monteclora_config.monteclora_at:
                self.mc_sampler_A = nn.ModuleDict({})
                self.mc_sampler_A[adapter_name] = MonteCLoRASampler(self.in_features, r, 
                                                monteclora_n = monteclora_config.monteclora_n, monteclora_m = monteclora_config.monteclora_m, use_entropy = monteclora_config.use_entropy,
                                                dirichlet_prior = monteclora_config.dirichlet_prior, sample_scaler = monteclora_config.sample_scaler,
                                                kl_loss_weight = monteclora_config.kl_loss_weight, mc_training = monteclora_config.mc_training, buffer_size = 10)
                # self.lora_A[adapter_name] = MonteCLoRALinear(self.in_features, r, 
                #                                 monteclora_n = monteclora_config.monteclora_n, monteclora_m = monteclora_config.monteclora_m, use_entropy = monteclora_config.use_entropy,
                #                                 dirichlet_prior = monteclora_config.dirichlet_prior, sample_scaler = monteclora_config.sample_scaler,
                #                                 kl_loss_weight = monteclora_config.kl_loss_weight, mc_training = monteclora_config.mc_training, buffer_size = 10)
                self.monteclora_A = True
                
            # if "lora_B" in monteclora_config.monteclora_at:
            #     self.mc_sampler_B = nn.ModuleDict({})
            #     self.mc_sampler_B[adapter_name] = MonteCLoRASampler(self.out_features, r, 
            #                                     monteclora_n = monteclora_config.monteclora_n, monteclora_m = monteclora_config.monteclora_m, use_entropy = monteclora_config.use_entropy,
            #                                     dirichlet_prior = monteclora_config.dirichlet_prior, sample_scaler = monteclora_config.sample_scaler,
            #                                     kl_loss_weight = monteclora_config.kl_loss_weight, mc_training = monteclora_config.mc_training, buffer_size = 10,
            #                                     start_update_at = 78)
            #     self.monteclora_B = True
        else:
    ...
    def forward(self, x: torch.Tensor, *args: Any, **kwargs: Any) -> torch.Tensor:
        ...
        if self.disable_adapters:
            ...
            for active_adapter in self.active_adapters:
                ...
                lora_A = self.lora_A[active_adapter]
                lora_B = self.lora_B[active_adapter]
                # instantiate mc_sampler and treat NaNs
                if self.use_monteclora:
                    if self.monteclora_A:
                        lora_A_vars, lora_A_wts = self.mc_sampler_A[active_adapter]()
                        if type(lora_A_vars) != int:
                            assert(
                                not torch.any(torch.isnan(lora_A_vars))
                            ), 'lora_A_var gave NaN'
                            assert(
                                not torch.any(torch.isnan(lora_A_wts))
                            ), 'lora_A_wts gave NaN'
                            updated_A = torch.nan_to_num(lora_A.weight.T + self.mc_sampler_A[active_adapter].sample_scaler * lora_A_vars, nan= 0.0)
                            updated_A = torch.nan_to_num(torch.einsum('i,ijk->jk', lora_A_wts, updated_A), nan=0.0)
                            lora_A.weight = nn.Parameter(updated_A.T)
                    if self.monteclora_B:
                        lora_B_vars, lora_B_wts = self.mc_sampler_B[active_adapter]()
                        if type(lora_B_vars) != int:
                            assert(
                                not torch.any(torch.isnan(lora_B_vars))
                            ), 'lora_B_var gave NaN'
                            assert(
                                not torch.any(torch.isnan(lora_B_wts))
                            ), 'lora_B_wts gave NaN'
                            updated_B = torch.nan_to_num(lora_B.weight + self.mc_sampler_A[active_adapter].sample_scaler * lora_B_vars, nan= 0.0)
                            updated_B = torch.nan_to_num(torch.einsum('i,ijk->jk', lora_B_wts, updated_B), nan=0.0)
                            lora_B.weight = nn.Parameter(updated_B)
